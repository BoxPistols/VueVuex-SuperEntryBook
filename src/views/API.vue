<template>
    <div>
        ...
        <input type="button" value="追加" @click="create"></div>
</template>

<script>
export default {
    data() {
        return { todos: [], new_todo: null }
    },
    created() {
        this.read()
    },
    methods: {
        ...
        create() {
            fetch('http://localhost:3000/todos', {
                method: 'POST',
                body: JSON.stringify({ name: this.new_todo }),
                headers: new Headers({ 'Content-type': 'application/json' })
            }).then(() => {
                this
                    .todos
                    .push({ name: this.new_todo })
                this.new_todo = ''
            })
        },
        ...
        // 削除
        remove(todo) {
            fetch(`http://localhost:3000/todos/${todo.id}`, { method: 'DELETE' }).then(
                () => {
                    this.todos = this
                        .todos
